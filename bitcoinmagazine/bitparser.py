...
def parse_article(url,par):
    ...
    clean_str_url = re.sub(r'https:\/\/bitcoinmagazine.com', '', url)
    ...
    # date_time_timestamp = dt.datetime.strptime(get_date_time(article, par), "%b %d, %Y  %I:%M %p EST").timestamp()
    date_time_timestamp = dt.datetime.strptime(get_date_time(article,par), "%b %d, %Y  %I:%M %p EST").replace(tzinfo=pytz.timezone('EST'))
